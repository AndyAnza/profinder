      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="relative mt-8">
          ...
          <Combobox as="div" value={selectedCity} onChange={setSelectedCity}>
            <Combobox.Label className="block text-sm font-medium leading-6 text-gray-900">Cities</Combobox.Label>
            <div className="relative mt-2">
              <Combobox.Input
                className="w-full rounded-md border-0 bg-white py-1.5 pl-3 pr-12 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
                onChange={(event) => setQuery(event.target.value)}
                ...
                <Combobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                  {cities.map((city) => (
                    ...
                    >
                      {({ active, selected }) => (
                        <>
                          <span className={classNames('truncate', selected && 'font-semibold')}>{city}</span>
                          {selected && (
                            <span
                              className={classNames(
                                'absolute inset-y-0 right-0 flex items-center pr-4',
        ...
        >
          {people.map((person) => (
            <li key={person.username}>
              ...
              <p className="text-sm leading-6 text-gray-600">{person.username}</p>
            </li>
          ))}
        </ul>
      </div>
